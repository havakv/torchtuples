        ...
        while watchdog.is_alive():
            ...
            try:
                if init_exception is not None:
                    ...
                    samples = collate_fn(dataset[batch_indices]) # changed in torchtuples
            except Exception:
...
class _DataLoaderIterSlice(object):
    ...
    def __next__(self):
        if self.num_workers == 0:  # same-process loading
            ...
            batch = self.collate_fn(self.dataset[indices]) # changed in torchtuples
            if self.pin_memory:
